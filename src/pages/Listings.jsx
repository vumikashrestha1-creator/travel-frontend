...
const calculateBadges = (listings) => {
  ...
  const cheapest = available.reduce((min, l) =>
    Number(l.discounted_price) < Number(min.discounted_price) ? l : min
  );
  ...
  if (fastest && !usedIds.has(fastest.id)) {
    ...
    if (next) { result.fastest = next.id; usedIds.add(next.id); }
  }
  ...
  if (bestValue && !usedIds.has(bestValue.id)) {
    ...
    if (next) { result.bestValue = next.id; usedIds.add(next.id); }
  }
...
const ListingBadge = ({ type }) => {
  const badges = {
    cheapest:  { label: "💰 Cheapest",     style: "bg-green-500 text-white"  },
    fastest:   { label: "⚡ Fastest",      style: "bg-blue-500 text-white"   },
    bestValue: { label: "⭐ Best Value",   style: "bg-yellow-500 text-white" },
    popular:   { label: "🔥 Selling Fast", style: "bg-orange-500 text-white" },
  };
  const badge = badges[type];
  if (!badge) return null;
  return (
    <span className={"text-xs font-bold px-2 py-1 rounded-full shadow-sm " + badge.style}>
      {badge.label}
    </span>
  );
};
...
const Listings = () => {
  ...
  const hasActiveFilters = Object.entries(filters).some(
    ([key, val]) =>
      val !== "" && val !== "ALL" && val !== "newest" && key !== "search"
  );
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex flex-wrap justify-center gap-3 mb-6">
          {[
            ...
            return (
              <span key={b.type} className={"text-xs px-3 py-1 rounded-full font-medium " + b.style}>
                {b.label}
              </span>
            );
          })}
        ...
        {message && (
          <div className="mb-6 px-4 py-3 rounded-lg bg-red-50 border border-red-200 text-red-700 flex justify-between items-center">
            <span>{message}</span>
            <button onClick={() => setMessage("")} className="text-sm underline ml-4">
              dismiss
            </button>
          </div>
        )}
                  ...
                  return (
                    <div
                      ...
                      className={
                        ...
                        (badgeType === "bestValue" ? "border-yellow-300" :
                         badgeType === "cheapest"  ? "border-green-300"  :
                         badgeType === "fastest"   ? "border-blue-300"   :
                         badgeType === "popular"   ? "border-orange-300" :
                         "border-gray-100")
                      }
                    >
                      {/* Clickable image */}
                        ...
                        <div className="border-t pt-3">
                          ...
                          {bookingId === listing.id ? (
                            <div className="space-y-2">
                              <div className="flex items-center gap-2">
                                <label className="text-gray-500 text-xs">Guests:</label>
                                <input
                                  type="number"
                                  min="1"
